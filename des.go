...
func Des(key string, message string, encrypt int, mode int, iv string, padding int) (string, error) {
    ...
    return result + tempresult, nil
}

func Des_createKeys(key string) ([]int, error) {
    ...
    if len(key) != 8 && len(key) < 24 {
        err := "key's length must be 8byte or bigger than 23 byte"
